 ...
 * under web/planner-spike, so nothing sensitive ships. Local SCRATCH is also
 * skipped (backup dirs, "- Copy"/"(N)" duplicates, *.rej) so untracked junk left
 * in the folder can't be published — see EXCLUDE_SCRATCH below.
 *
...
const EXCLUDE_DIR = /[\\/](portraits-borrowed|node_modules|\.git)([\\/]|$)/;
// skip LOCAL SCRATCH living UNTRACKED in this folder — cpSync copies untracked files too, so a backup
// dir / Windows "- Copy" / "name(2).png" duplicate / leftover *.rej (from `git apply --reject`) would
// otherwise get PUBLISHED. This already happened once (an "Altar Backup/" folder shipped to the live site).
const EXCLUDE_SCRATCH = /[\\/][^\\/]*[Bb]ackup[^\\/]*([\\/]|$)| - Copy|\(\d+\)\.[^.\\/]+$|\.rej$/;
const EXCLUDE = (src) => EXCLUDE_DIR.test(src) || EXCLUDE_SCRATCH.test(src);
...
try {
  ...
  cpSync(here, dest, { recursive: true, filter: (src) => !EXCLUDE(src) });
